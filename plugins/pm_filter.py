...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("⚠️Bro Search Your Own File , Don't Click Others Requested Files😞", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🤖 sᴇᴛᴛɪɴɢs 🤖', callback_data='sts')
        ], [
    ...
    elif query.data == "covd":
        buttons = [[
            InlineKeyboardButton('👩‍🦯 ʙᴀᴄᴋ', callback_data='help')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.COVD_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            if btn is not None:
                try:
                    if fileid == "None":
                        if btn == "[]":
                            u = await client.send_message(group_id, reply_text, disable_web_page_preview=True)
                            await asyncio.sleep(90)
                            await u.delete()
                        else:
                            button = eval(btn)
                            p = await client.send_message(
                                group_id,
                                reply_text,
